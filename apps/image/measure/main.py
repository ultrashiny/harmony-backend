...
def rescale(startpoint, width, points):
    scaled_points = []
    for point in points:
        scaled_x = (point[0] - startpoint[0]) * 512 / width
        scaled_y = (point[1] - startpoint[1]) * 512 / width
...
def create_eye_spacing_ratio_image(points, RLs, DIR, index, canvas):
    ...
    c3 = getIntersection((points[16][0], getVertical(points[16][0], RLs[23])), RLs[23])
...
#15
def create_eye_aspect_ratio_image(points, RLs, DIR, index, canvas):
    indexes = [10, 14, 16, 11]
    RLIndexes = []
    (TL, BR), W = GetFeatureArea(points, indexes)    # TopLeft, BottomRight, Width
    crop = GetAreaImage(canvas, TL, BR)
...
def create_gonion_to_mouth_relationship_image(points, RLs, DIR, index, canvas):
    ...
    # TEMP POINT
    temp = getIntersection((points[46][0], getVertical(points[46][0], RLs[2])), RLs[2])

    dotLines = []
    solidLines = [(points[46][0], temp)]
    drawPoints = [points[46][0], points[49][0], temp]
...
def create_recession_relative_to_frankfort_plane_image(points, RLs, DIR, index, canvas):
    ...
    # TEMP POINT
    temp = getIntersection((points[35][0], getVertical(points[35][0], RLs[5])), RLs[5])

    dotLines = []
    solidLines = [(points[35][0], temp)]
    drawPoints = [points[35][0], points[50][0], temp]
...
async def createReportImages(id, f_canvas, s_canvas, points, lines):
    ...
    create_cheekbone_height_image(points, RLs, DIR, 5, f_canvas)
    create_total_facial_height_to_width_ratio_image(points, RLs, DIR, 6, f_canvas)
    create_bigonial_width_image(points, RLs, DIR, 7, f_canvas)
    create_chin_to_philtrum_ratio_image(points, RLs, DIR, 8, f_canvas)
    create_Neck_width__image(points, RLs, DIR, 9, f_canvas)
    create_mouth_width_to_nose_width_ratio_image(points, RLs, DIR, 10, f_canvas)
    ...
    create_ipsilateral_alar_angle_image(f_url, points, DIR, 20)
    create_medial_canthal_angle_image(f_url, points, DIR, 21)
